...
def check_local(args: argparse.Namespace) -> bool:
    ok_all = True
    ok, text = run([sys.executable, "scripts/checks/check_repo_ready.py"], timeout=90)
    ok_all &= print_block(ok, "repository readiness with current Python", text if not ok else "")

    conda = conda_available()
    ok_all &= print_block(bool(conda), "conda available", conda or "Run scripts/env/install_miniforge.sh")
    if not conda:
        return ok_all
    ...
    checks = [
        (
            ...
            [conda, "run", "-n", "varmdyn_env", "python", "scripts/checks/check_repo_ready.py"],
            120,
        ),
        (
...
def check_hpc(args: argparse.Namespace) -> bool:
    ...
    bridge_cmd = [sys.executable, "scripts/checks/check_hpc_bridge.py"]
    if socket:
