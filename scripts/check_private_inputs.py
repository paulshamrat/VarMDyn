...
def remote_check(host: str | None, path: str | Path, *, kind: str = "path") -> Check:
    if not host:
    ...
    proc = subprocess.run(ssh, text=True, capture_output=True, check=False)
...
def network_checks(*, remote: bool) -> list[Check]:
    ...
    if project:
        checks.append(remote_check(host, project, kind="dir"))
    if work:
        checks += [
            remote_check(host, work, kind="dir"),
            remote_check(host, Path(work) / "06_step1_CDKL5_with_lab_outputs.py", kind="file"),
            remote_check(host, Path(work) / "07_compare_networks_all_variants.py", kind="file"),
            remote_check(host, Path(work) / "TutorialData_CDKL5", kind="dir"),
            remote_check(host, Path(work) / "TutorialResults_CDKL5", kind="dir"),
        ]
    return checks


def print_checks(checks: list[Check]) -> int:
    width = max(len(c.label) for c in checks) if checks else 10
    failed = False
    for check in checks:
        status = "OK" if check.ok else "MISSING"
        print(f"[{status}] {check.label:<{width}} : {check.detail}")
...
def main() -> int:
    ...
    if args.module == "network":
        return print_checks(network_checks(remote=args.remote))
