#!/usr/bin/env python3
"""Build separate C-alpha and COM review SVGs for Figure 2."""
...
CALPHA_SVG = ROOT / "fig2_variant_context_calpha_only_review.svg"
COM_SVG = ROOT / "fig2_variant_context_com_only_review.svg"
...
def section_positions(panel_keys: list[str]) -> dict[str, tuple[int, int]]:
    row1_y = MARGIN + HEADER_GAP
    row2_y = row1_y + PANEL_H + GUTTER
    col1_x = MARGIN
...
def draw_section(
    ...
    canvas_h = MARGIN * 2 + HEADER_GAP + PANEL_H * 2 + GUTTER
    positions = section_positions(list(panels))
    parts = [
    ...
    for key, (title, path) in panels.items():
        ...
        if key == "A":
            ...
            draw_labels(parts, x, y, OVERVIEW_LABELS[section_key])
        else:
...
def main() -> None:
    draw_section(CALPHA_SVG, "calpha", CALPHA_PANELS, CALPHA_LEGEND, CALPHA_LABELS)
    draw_section(COM_SVG, "com", COM_PANELS, COM_LEGEND, COM_LABELS)
    print(CALPHA_SVG)
    print(COM_SVG)
