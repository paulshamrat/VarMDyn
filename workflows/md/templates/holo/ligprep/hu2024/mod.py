...
import sys
import numpy as np
from parmed.topologyobjects import Cmap, CmapType
def Check(Path):
    try:
    ...
    Top = AmberParm(Path)
    if "ATP" not in Top.parm_data["RESIDUE_LABEL"] and\
        "GTP" not in Top.parm_data["RESIDUE_LABEL"] and\
        "ADP" not in Top.parm_data["RESIDUE_LABEL"] and\
        "GDP" not in Top.parm_data["RESIDUE_LABEL"]:
            print("ATP or GTP or ADP or GDP is Not in the TOP, SO NO Need Modification !!!\n")
            sys.exit(1)
    ...
    if "GTP" in Top.parm_data["RESIDUE_LABEL"]:
        name.append("GTP")
    if "ADP" in Top.parm_data["RESIDUE_LABEL"]:
        name.append("ADP")
    if "GDP" in Top.parm_data["RESIDUE_LABEL"]:
        name.append("GDP")


    cmap_name = ["O3B","PB","O3A","PA","O5'"]
    index = []
    residues = [residue for residue in Top.residues if residue.name in name]
...
def Gen_cmap(Path, index, method, dis_cmap=False):
    cmap_matrix_HF = [
    ...
    if not dis_cmap:
        ...
        if "CMAP_COUNT" in [key for key, value in Top.parm_comments.items()]:
            ...
            Top.add_flag(flag_name = f"CMAP_PARAMETER_{last+1:02}", flag_format = "8F9.5",data=cmap_matrix,comments=["Polyphosphate CMAP"])
            ...
            Top.add_flag(flag_name = "CMAP_PARAMETER_01", flag_format = "8F9.5",data=cmap_matrix,comments=["Polyphosphate CMAP"])
            for m in range(shape[0]):
...
def load_cmap_info(top):
#    if not top.has_cmap: return
    del top.cmaps[:]
    del top.cmap_types[:]
    resolution_key = f"{top._cmap_prefix}CMAP_RESOLUTION"
    parameter_key = f"{top._cmap_prefix}CMAP_PARAMETER_{{:02d}}"
    for i in range(top.parm_data["CMAP_COUNT"][1]):
        resolution = top.parm_data[resolution_key][i]
        grid = top.parm_data[parameter_key.format(i+1)]
        cmts = top.parm_comments[parameter_key.format(i+1)]
        top.cmap_types.append(CmapType(resolution, grid, cmts, list=top.cmap_types))
    it = iter(top.parm_data[top._cmap_prefix + 'CMAP_INDEX'])
    for i, j, k, l, m, n in zip(it, it, it, it, it, it):
        top.cmaps.append(
            Cmap(top.atoms[i-1], top.atoms[j-1], top.atoms[k-1],
                    top.atoms[l-1], top.atoms[m-1], top.cmap_types[n-1])
        )
...
def main():
    parser = argparse.ArgumentParser(description="This is a Python script for Modifying the TOP File Containing ATP, GTP, ADP, GDP and Mg2+")
    parser.add_argument("-dis_cmap", type=bool, choices=[True,False],default=False, help="Whether to Enable CMAP Potential")
    ...
    if method not in ["HF", "B3LYP"]:
        ...
        sys.exit(1)
    index = Check(Path=path)
    top = Gen_cmap(Path=path, index=index, method=method, dis_cmap=dis_cmap)
    top = Modify_lj(top)
    load_cmap_info(top)
