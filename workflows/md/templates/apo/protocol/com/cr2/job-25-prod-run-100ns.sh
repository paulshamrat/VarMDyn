...
echo "Job-25 started on: `date`"
${CMD} -O -i  ${PROTOCOL}/25md.in \
-o      ${TRAJDIR}/25md.mdout \
-p      ${LEAPDIR}/${PDB}.${STATE}.wat.leap.prmtop \
-c      ${TRAJDIR}/24md.restrt \
-ref    ${TRAJDIR}/14mi.restrt \
-x      ${TRAJDIR}/25md.mdcrd.nc \
-inf    ${TRAJDIR}/25md.info \
-r      ${TRAJDIR}/25md.restrt
