...
def md_root(args: argparse.Namespace) -> Path:
    raw = (
        args.md_root
        or os.environ.get("VARMDYN_MD_SOURCE_ROOT")
        or os.environ.get("VARMDYN_MD_GENERATION_ROOT", REPO_ROOT / "data" / "md")
    )
    return Path(os.path.expandvars(str(raw))).expanduser()


def default_out_root(md_root_value: Path) -> Path:
    env_mdan = os.environ.get("VARMDYN_MDAN_OUTPUT_ROOT")
    if env_mdan:
        return Path(os.path.expandvars(env_mdan)).expanduser()
    if md_root_value.name == "md":
        return md_root_value.parent / "mdan"
    return Path(os.environ.get("VARMDYN_RUN_ROOT", REPO_ROOT / "data")) / "mdan"
...
def rms_log_root(args: argparse.Namespace) -> Path:
    return out_root(args) / "rms" / "logs" / args.state
...
def aggregate_variant(args: argparse.Namespace) -> int:
    ...
    aggregate_series(rmsd_files, "frame", out / "rms" / "rmsd" / state / variant.name / "rmsd_bb_mean_sd.csv")
    aggregate_series(rmsf_files, "residue", out / "rms" / "rmsf" / state / variant.name / "rmsf_mean_sd.csv")
...
cd "$REPO_ROOT"
python workflows/mdan/rms/runner.py aggregate \
  --state "$STATE" \
...
def expected_outputs(args: argparse.Namespace, variant: Path) -> list[Path]:
    ...
    base = out_root(args)
    return [
        base / "rms" / "rmsd" / state / variant.name / "rmsd_bb_mean_sd.csv",
        base / "rms" / "rmsf" / state / variant.name / "rmsf_mean_sd.csv",
    ]
...
def plan(args: argparse.Namespace) -> int:
    ...
    print("outputs=rms/rmsd/<state>/<variant>/rmsd_bb_mean_sd.csv,rms/rmsf/<state>/<variant>/rmsf_mean_sd.csv")
    return 0
