...
export NUMBA_CACHE_DIR="${NUMBA_CACHE_DIR:-$PWD/runs/numba_cache}"
...
if conda env list | awk '{print $1}' | grep -qx "${ENV_NAME}"; then
  if PYTHONNOUSERSITE=1 NUMBA_CACHE_DIR="${NUMBA_CACHE_DIR}" conda run -n "${ENV_NAME}" python -c "import dynetan, traitlets, ipywidgets, MDAnalysis, networkx, parmed; import importlib.metadata as md; ok = md.version('dynetan') == '2.2.2' and md.version('MDAnalysis').startswith('2.9.') and md.version('ParmEd').startswith('4.3.'); raise SystemExit(0 if ok else 1)" >/dev/null 2>&1; then
