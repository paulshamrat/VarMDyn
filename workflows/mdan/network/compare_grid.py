...
def make_panel(
    ...
    for ax, variant in zip(axes.flatten(), VARIANTS):
        img = open_variant_image(render_dir, variant, suffix)
...
def main() -> None:
    ...
    apo_render_dir = Path(os.environ.get("VARMDYN_NETWORK_APO_RENDER_DIR", data_root / "render/apo")).expanduser()
    holo_render_dir = Path(os.environ.get("VARMDYN_NETWORK_HOLO_RENDER_DIR", data_root / "render/holo")).expanduser()

    out_dir = apo_render_dir.parent
    out_dir.mkdir(parents=True, exist_ok=True)

    make_panel(
        apo_render_dir,
        "apo",
        out_dir / "network_apo_pathway_tight.png",
        out_dir / "network_apo_pathway_tight.jpg",
        show_legend=True,
    )
    make_panel(
        holo_render_dir,
        "holo",
        out_dir / "network_holo_pathway_tight.png",
        out_dir / "network_holo_pathway_tight.jpg",
        show_legend=True,
    )
    make_combined_preview(
        apo_render_dir,
        holo_render_dir,
        out_dir / "network_compare_preview_2col_tight.png",
    )
    print("[OK] wrote tight apo+holo panels and local comparison preview")
