...
This script keeps the manuscript-style network replay in one public-facing CLI:
...
def main() -> int:
    ...
    p = sub.add_parser("hpc-stage")
    p = sub.add_parser("hpc-submit")
    p = sub.add_parser("hpc-status")
    p.add_argument("--job-id")
    p = sub.add_parser("hpc-wait")
    p.add_argument("--job-id")
    p.add_argument("--poll-seconds", type=int, default=120)
    p = sub.add_parser("hpc-compare")
    p = sub.add_parser("hpc-fetch")
    p.add_argument(
        "--outdir",
        default=str(REPO_ROOT / "data" / "network" / "replay" / "apo"),
        help="local ignored directory for fetched lightweight network outputs",
    )
    p = sub.add_parser("hpc-run")
    ...
    if args.command == "prepare":
        ...
        run_full(args)
    elif args.command == "hpc-stage":
        hpc_stage()
    elif args.command == "hpc-submit":
        hpc_submit()
    elif args.command == "hpc-status":
        hpc_status(resolve_hpc_job_id(args.job_id))
    elif args.command == "hpc-wait":
        hpc_wait(resolve_hpc_job_id(args.job_id, required=True), args.poll_seconds)
    elif args.command == "hpc-compare":
        hpc_compare()
    elif args.command == "hpc-fetch":
        hpc_fetch(Path(args.outdir))
    elif args.command == "hpc-run":
        hpc_run(args)
    return 0
