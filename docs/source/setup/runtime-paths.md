...
## 2. Optional Note: Google Colab Paths

VarMDyn is documented primarily for local workstation and HPC use. If you are
running in a Google Colab notebook, mount your Google Drive and set the roots to
your Google Drive repository directory to keep outputs persistent:
...
legacy MD analysis sources. In Paul's local/private checkout, ignored local path
files can provide these values automatically. For a generic setup, set them only
for workflows that use MD trajectories, RMSD/RMSF source files, displacement
tables, DyNetAn outputs, or an HPC run folder:
